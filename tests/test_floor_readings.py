...
def test_fetch_lists_all_building_floors_with_system_totals(login_as, db_session):
    ...
    # two completed runs on 1st floor today -> summed; Ground floor idle -> 0
    _run(db_session, "A185-2", "A-185", "1st floor", today, 10)
    _run(db_session, "A185-3", "A-185", "1st floor", today, 5)
    db_session.commit()
    ...
    assert body["reading_date"] == today.isoformat()
...
def test_submit_upserts_and_recomputes_system(login_as, db_session):
    ...
    _asset(db_session, "A185-1", "A-185", "Ground floor")
    _asset(db_session, "A185-2", "A-185", "1st floor")
    _run(db_session, "A185-2", "A-185", "1st floor", today, 12)
    db_session.commit()
    ...
    resp2 = c.post("/floor-readings", json={"rows": [{"floor": "1st floor", "meter_reading": 260.0}]})
...
def test_fetch_echoes_saved_meter_reading(login_as, db_session):
    ...
    db_session.add(MtFloorUtilityReading(
        building="A-185", floor="Ground floor", reading_date=today,
        meter_reading=77.0, daily_kwh=0,
    ))
    db_session.commit()
    c = login_as(role="TECHNICIAN", location="A-185")
    body = c.get("/floor-readings/system").json()
...
def test_descriptive_location_resolves(login_as, db_session):
    ...
    today = datetime.utcnow().date()
    _asset(db_session, "A185-1", "A-185", "Ground floor")
    _run(db_session, "A185-1", "A-185", "Ground floor", today, 4)
    db_session.commit()
...
def test_head_with_building_param_works(login_as, db_session):
    ...
    _asset(db_session, "A185-1", "A-185", "Ground floor")
    _asset(db_session, "W202-1", "W-202", "Terrace")
    _run(db_session, "W202-1", "W-202", "Terrace", today, 9)
    db_session.commit()
    c = login_as(role="HEAD", location="A-185")
    resp = c.get("/floor-readings/system", params={"building": "W202"})
    assert resp.status_code == 200, resp.text
    body = resp.json()
